...
export class TableFactory {
    ...
    createTableColumnFromConfig(
        ...
        if (columnConfig.visible || columnConfig.visible == null) {
            ...
            if (columnType) {
                if (columnType.format) {
                    column.format = columnType.format;
                }
                if (columnType.component) {
                    column.component = columnType.component;
                }
                if (columnType.columnClass) {
                    column.columnClass = columnType.columnClass;
                }
                column.classList = columnType.classList ? columnType.classList : [''];
            } else {
                switch (columnConfig.type) {
                    case SmartTableColumnType.DateTime: {
                        column.format = (value) => this.datePipe.transform(value, format || 'dd/MM/yyyy - hh:mm');
                        break;
                    }
                    case SmartTableColumnType.Date: {
                        column.format = (value) => this.datePipe.transform(value, format || 'dd/MM/yyyy');
                        break;
                    }
                }
            }
