...
export class SmartTableComponent implements OnInit, OnDestroy {
  ...
  /** @internal */
  orderBy$: Subject<OrderBy> = new BehaviorSubject<OrderBy>(
    SMARTTABLE_DEFAULT_OPTIONS.defaultSortOrder
  );
  /** @internal */
  ...
  ngOnInit(): void {
    this.configuration$ = concat(
      ...
      this.customConfiguration$.pipe(
        ...
        switchMap((customConfig) =>
          combineLatest(of(customConfig), this.configuration$).pipe(first())
        ),
      ...
      map((config) =>
        config.options.persistTableConfig
    ...
    this.allColumns$ = this.configuration$.pipe(
      tap((config) =>
        this.resetOrderBy(
          config && config.options && config.options.defaultSortOrder
        )
      ),
      map((config: SmartTableConfig) =>
        config.columns.map((c) =>
          this.factory.createTableColumnFromConfig(
            c,
            this.columnTypes,
            SMARTTABLE_DEFAULT_OPTIONS.columnDateFormat
          )
        )
      ),
      startWith([]),
    ...
    this.persistInStorage$ = this.toggleHideColumn$.pipe(
      ...
      switchMap(() =>
        combineLatest(this.configuration$, this.selectableColumns$)
      ),
      filter(
        ([config, selectableColumns]: [SmartTableConfig, TableColumn[]]) =>
          config.options.persistTableConfig === true &&
          !!config.options.storageIdentifier &&
          !!selectableColumns
      ),
      map(([configuration, selectableColumns]) => {
        configuration.columns.map((column) => {
          const found = (selectableColumns as Array<TableColumn>).find(
            (c) => c.value === column.key
          );
          if (found) {
    ...
    this.visibleColumns$ = merge(
      ...
      this.toggleHideColumn$.pipe(
        switchMap(() =>
          combineLatest(this.allColumns$, this.selectableColumns$).pipe(take(1))
        ),
        map(([columns, selectableColumns]) => {
          return columns.map((column) => {
            const found = selectableColumns.find(
      ...
      combineLatest(this.allColumns$, this.configuration$).pipe(
        map(([allColumns, configuration]) => {
          return (allColumns as TableColumn[]).filter((column) => {
            const config = configuration.columns.find(
              (c) => c.key === column.value
            );
        ...
        switchMap((v) =>
          this.selectableColumns$.pipe(
            ...
            map((selectableColumns) => {
              const i = (selectableColumns as TableColumn[]).findIndex(
                (c) => c.value === v.key
              );
              if (i > -1) {
    ...
    this.optionalFilters$ = this.configuration$.pipe(
      filter(
        (config: SmartTableConfig) =>
          !!config && Array.isArray(config.filters) && config.filters.length > 0
      ),
      map((config: SmartTableConfig) =>
        this.setupFilter(config.filters, SmartTableFilterDisplay.Optional)
      ),
      startWith([]),
      shareReplay(1)
    );
    this.optionalFilters$ = merge(
      this.optionalFilters$,
      onFilter(this.optionalFilters$)
    );
    ...
    this.visibleFilters$ = this.configuration$.pipe(
      ...
      map((config: SmartTableConfig) =>
        this.setupFilter(config.filters, SmartTableFilterDisplay.Visible)
      ),
      startWith([]),
      shareReplay(1)
    );
    this.visibleFilters$ = merge(
      this.visibleFilters$,
      onFilter(this.visibleFilters$)
    );

    this.genericFilter$ = this.configuration$.pipe(
      map((config: SmartTableConfig) =>
        config.filters.find(
          (f) => f.display === SmartTableFilterDisplay.Generic
        )
      ),
      filter((f) => !!f),
      map((genericFilter) => {
    ...
    this.dataQuery$ = combineLatest(
      ...
      map(
        ([filters, configuration, orderBy]: [
          ...
          const sortColumn = configuration.columns.find(
  ...
  public getConfiguration(): Observable<SmartTableConfig> {
    return this.dataService
      .getConfiguration(this.apiUrl, this.httpHeaders)
      .pipe(
        first(),
  ...
  public getLocalStorageObject(configuration: SmartTableConfig) {
    const json = this.localstorageService.storage.getItem(
    ...
    try {
      ...
      try {
        ...
        console.warn(
          "Warning: could not parse smart table columns from storage!"
        );
      }
      ...
      try {
        ...
        console.warn(
          "Warning: could not parse smart table sort order from storage!"
        );
      }
  ...
  public setupFilter(
    ...
    type: SmartTableFilterDisplay
  ): Array<SmartTableFilter> {
    return filters
      .filter((f) => f.display === type)
      .map((filterConfig) =>
        this.factory.createSmartFilterFromConfig(filterConfig)
      );
  }

  protected resetOrderBy(defaultSortOrder?) {
    const sortOrder =
      defaultSortOrder || SMARTTABLE_DEFAULT_OPTIONS.defaultSortOrder;
    if (sortOrder) {
      this.orderBy$.next(sortOrder);
    }
  }

  createDataQueryFilters(
  ...
  public exportToExcel() {
    ...
    this.dataQuery$
      .pipe(
        first(),
        switchMap((dataQuery) =>
          this.dataService.getAllData(this.apiUrl, this.httpHeaders, dataQuery)
        ),
